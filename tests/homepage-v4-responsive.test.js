...
test('mobile 3D hero preserves the square source frame instead of stretching it', () => {
  ...
  assert.match(html, /@media \(max-width:768px\)[\s\S]*?html\.fluid \.hero__media\{height:auto;aspect-ratio:1\/1\}/);
  assert.match(html, /@media \(max-width:768px\)[\s\S]*?html\.fluid \.hero__v3d canvas\{width:auto;height:90%;aspect-ratio:1\/1;margin:5% auto 0\}/);
  assert.match(html, /@media \(max-width:768px\)[\s\S]*?html\.fluid \.hero__3dhint\{left:50%;top:32px\}/);
  assert.match(html, /@media \(max-width:768px\)[\s\S]*?html\.fluid \.hero__pill\{left:50%;right:auto;transform:translateX\(-50%\);bottom:36px/);
  assert.doesNotMatch(html, /html\.fluid \.hero__pill\{bottom:4px\}/);
